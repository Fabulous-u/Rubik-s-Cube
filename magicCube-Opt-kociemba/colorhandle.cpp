...
void colorhandle::cameraInit() {
	...
	rfsh->stop();
	capture_f.set(3/*CV_CAP_PROP_FRAME_WIDTH*/, IMAGEWID);
	capture_f.set(4/*CV_CAP_PROP_FRAME_HEIGHT*/, IMAGEHEIGHT);
	capture_b.set(3/*CV_CAP_PROP_FRAME_WIDTH*/, IMAGEWID);
	capture_b.set(4/*CV_CAP_PROP_FRAME_HEIGHT*/, IMAGEHEIGHT);
...
//以下为魔方视觉第二套方案
void colorhandle::point_Config(int x_, int y_, double size_) {

	static double temp_size = 1.15;
	temp_size += size_;
	...
	for_R_area_corner.clear();

	for_R_area_corner.push_back(_L_U[0]);
	for_R_area_corner.push_back(_C_U[0]);
	for_R_area_corner.push_back(_C_D[0]);
	for_R_area_corner.push_back(_L_D[0]);
...
void colorhandle::point_Config2(int x_, int y_, double size_) {

	//以下为魔方视觉第二套方案

	static double temp_size = 1.1;
	temp_size += size_;
...
void colorhandle::point_Config3() {//初始调节位置
   ...
   /* short Xpos = 27;
	...
	double temp_width = IMAGEAREA_RL_WIDTH * 1.15;
	double temp_height = IMAGEAREA_HEIGHT * 1.15;

	/*  short Xpos2 =82;
	  short Ypos2 = 101 -IMAGE_TOUSHI_FAR;*/
	double temp_width2 = IMAGEAREA_RL_WIDTH * 1.1;
	double temp_height2 = IMAGEAREA_HEIGHT * 1.1;
...
cv::Mat colorhandle::getFrame_b() {
	...
	if (!camera_b_stop) {
		...
		GaussianBlur(frame_b, imageout, cv::Size(3, 3), 0, 0);
		//       imageout = frame_b;
		return imageout;
	}
	return imageout;
...
cv::Vec3b colorhandle::getPartAeraHSV(const cv::Mat tempIge, cv::Point target) {        //获取方块颜色
	...
	area_pix[0] = (unsigned char)(vg / 4);
	area_pix[1] = (unsigned char)(vb / 4);
	area_pix[2] = (unsigned char)(vr / 4);
	std::string filename = "";
	//filename = "H:" + std::to_string(area_pix[0]) + "S:" + std::to_string(area_pix[1]) + std::to_string(area_pix[2]) + "E" + std::to_string(rand() % 1000) + ".tiff";
	filename = std::to_string(rand()) + "asfghjklmnbvcxz"[rand() % 12] + ".tiff";
	cv::imwrite( filename,tempIge(cv::Rect(target.x - 3, target.y - 3, 6, 6)));
...
cv::Mat colorhandle::mwrap_area_handle_last(cv::Mat imgGet) {
...
void colorhandle::set_Camera_params() {
	...
	capture_f.set(15/*CV_CAP_PROP_EXPOSURE*/, Camera_EXPOSURE_param);
	capture_f.set(12/*CV_CAP_PROP_SATURATION*/, Camera_SATURATION_param);
	capture_f.set(11/*CV_CAP_PROP_CONTRAST*/, Camera_CONTRAST_param);
	capture_f.set(10/*CV_CAP_PROP_BRIGHTNESS*/, Camera_BRIGHTNESS_param);
	capture_f.set(13/*CV_CAP_PROP_HUE*/, Camera_HUE_param);

	capture_b.set(15/*CV_CAP_PROP_EXPOSURE*/, Camera_EXPOSURE_param);
	capture_b.set(12/*CV_CAP_PROP_SATURATION*/, Camera_SATURATION_param);
	capture_b.set(11/*CV_CAP_PROP_CONTRAST*/, Camera_CONTRAST_param);
	capture_b.set(10/*CV_CAP_PROP_BRIGHTNESS*/, Camera_BRIGHTNESS_param);
	capture_b.set(13/*CV_CAP_PROP_HUE*/, Camera_HUE_param);
