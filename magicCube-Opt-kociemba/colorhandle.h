...
//    std::vector<cv::Point2f>D_r_area_corner;
    ...
    std::vector<cv::Point2f>U_L_area_corner;
    std::vector<cv::Point2f>U_R_area_corner;
    void point_Config(int x_ =0,int y_ =0,double size_ =0 );

    std::vector<cv::Point2f>D_L_area_corner;
    std::vector<cv::Point2f>D_R_area_corner;

    void point_Config2(int x_=0 ,int y_=0 ,double size_ =0);

	std::vector<cv::Point2f>for_R_area_corner; //这里基于之前坐标问题设置
...
private:
    ...
    short EXPOSURE_ = -15;
    short SATURATION_ = 120;
    short CONTRAST_ = 12;
    short BRIGHTNESS_ = 15;
    short HUE_param_   = -203;
...
//    bool c_u_line(short x,short y);
    short Xpos = 66;
    short Ypos = 78 - IMAGE_TOUSHI_LCOSE;
    short Xpos2 = 108;
    short Ypos2 = 42 - IMAGE_TOUSHI_FAR;
